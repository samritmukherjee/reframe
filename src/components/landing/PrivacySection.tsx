...
function DataFlowChart() {
  ...
  return (
    <div ref={ref} className="space-y-3 mt-6">
      <p className="text-[10px] uppercase tracking-widest text-[var(--muted)]">Data flow breakdown</p>
      {bars.map((b, i) => (
        <div key={i} className="flex items-center gap-3">
          <span className="text-[11px] text-[var(--muted)] w-32 shrink-0 text-right">{b.label}</span>
          <div className="flex-1 h-4 rounded-full bg-[var(--bg)] ring-1 ring-[var(--border)]/30 overflow-hidden">
            <div
              className={`h-full rounded-full ${b.color} transition-all duration-1000 ease-out`}
              style={{ width: animate ? `${b.value}%` : "0%" }}
            />
          </div>
          <span className="text-[11px] font-mono text-[var(--muted)] w-10">{b.value}%</span>
        </div>
      ))}
    </div>
  );
...
const trustCards = [
  ...
  {
    icon: <FiCpu className="h-4.5 w-4.5" />,
  ...
  {
    icon: <FiCloudOff className="h-4.5 w-4.5" />,
    title: "No uploads",
  ...
  {
    icon: <FiEyeOff className="h-4.5 w-4.5" />,
    title: "No tracking",
  ...
  {
    icon: <FiKey className="h-4.5 w-4.5" />,
        ...
        <div className="grid grid-cols-1 gap-20 lg:grid-cols-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {trustCards.map((card, i) => (
              <div
                key={i}
                className={`rounded-xl bg-[var(--surface)]/50 backdrop-blur-sm p-4 ring-1 ${card.ring} hover:ring-[var(--border)] transition-all duration-300 ${i === 0 ? "sm:col-span-2" : ""}`}
              >
                <div className={`${card.accent} mb-3`}>{card.icon}</div>
                <h4 className="text-sm font-medium text-[var(--text)] mb-1">{card.title}</h4>
